...
if __name__ == "__main__":
    ...
    try:
        for result_dict in tqdm(
            pool.imap_unordered(worker, all_files, chunksize=10), total=len(all_files), unit="files"
        ):
            ...
            counterdict_before = sum_dicts(counterdict_before, result_dict["counterdict_before"])
            counterdict_after = sum_dicts(counterdict_after, result_dict["counterdict_after"])
            counterdict_lang = sum_dicts(counterdict_lang, result_dict["counterdict_lang"])
